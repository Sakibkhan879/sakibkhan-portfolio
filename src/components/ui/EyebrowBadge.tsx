...
export function EyebrowBadge({ children, className = "" }: Props) {
  return (
    <span
      ...
      style={{
        boxShadow:
          "inset 0 1px 0 rgba(255,255,255,0.06), 0 0 24px -8px color-mix(in oklab, var(--color-accent) 25%, transparent)",
      }}
    >
      <span className="inline-block h-1.5 w-1.5 rounded-full bg-accent shadow-accent-glow" />
      {children}
    </span>
  );
}
